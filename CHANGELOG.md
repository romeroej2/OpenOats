...
- **Obsidian vault integration** - OpenCassava can now connect directly to a
  local Obsidian vault, index selected vault folders for suggestions, and keep
  canonical meeting knowledge inside the vault instead of only relying on a
  standalone notes folder.
- **Canonical meeting publishing** - when a session stops, OpenCassava now
  generates a final note and publishes it into
  `OpenCassava/Meetings/YYYY/MM/<session_id>.md`, with a matching transcript
  companion written to `OpenCassava/Transcripts/YYYY/MM/<session_id>.md`.
- **Incremental vault indexing** - the knowledge base now reuses cached
  embeddings for unchanged files, removes deleted files from the index, returns
  vault-relative source paths, and excludes `.obsidian` plus raw transcript
  companions from retrieval.
- **Bidirectional note links** - published summaries and transcript companions
  now link to each other with explicit vault-relative Obsidian wikilinks so the
  session summary and transcript stay connected.
- **Issue coverage** - this release fixes
  [#31](https://github.com/romeroej2/OpenCassava/issues/31) by shipping the
  end-to-end Obsidian workflow for knowledge retrieval and canonical notes.

### UI & workflow

- **Knowledge & Notes settings** - the General settings tab is now centered on a
  single Obsidian vault workflow with knowledge folders, default template
  selection, vault indexing status, and clearer language around what is indexed
  and what is published.
- **Publishing status feedback** - after recording stops, the desktop app now
  surfaces background status for generating the summary, publishing to
  Obsidian, and reporting publish or reindex failures.
- **Suggestion source context** - suggestion cards now show vault-relative note
  paths and, when available, the matched header context for the retrieved note.
- **Update availability indicator** - the desktop UI now checks the installed
  app version against the latest GitHub release and shows a persistent status
  badge with a direct download link when an update is available. Tracks issue
  `#25`.
...
- **Suggestion quick controls** - added an easy-to-find live suggestions toggle
  and cadence control directly in the main app UI instead of burying cadence in
  Settings only.
- **Suggestion disable setting** - added a persisted `suggestionsEnabled`
  setting so users can fully turn off live suggestion checks and overlay
  surfacing.

### Save recordings

- **Record Audio toggle** - a new checkbox in the ControlBar lets users opt in
  to recording audio before each session starts. The toggle is disabled
  mid-session so the choice is locked in at call start.
- **Per-channel WAV export** - mic and system audio are streamed to separate
  temp WAV files (16kHz mono f32) in real time during the session with no
  overhead when recording is off.
- **Save modal** - when a recorded session ends, a modal offers three
  checkboxes: *Microphone (you)*, *System audio (them)*, and *Merged (both
  channels mixed)*. Clicking Save Selected opens a file picker for each chosen
  file in sequence.
...
### Local AI defaults & docs

- **LM Studio-safe embedding default** - the OpenAI-compatible embedding default
  now uses `text-embedding-nomic-embed-text-v1.5` on `http://127.0.0.1:1234`,
  matching the known-good LM Studio embedding model for local KB indexing.
- **LM Studio setup docs** - the main README and LM Studio setup guide now
  document the recommended embedding model, endpoint expectations, and the
  distinction between LM Studio/OpenAI-compatible embeddings and Ollama
  embeddings.
...
- **Root lint entrypoint** - added a repo-root `npm run lint` / `npm run lint:fix`
  ...
  command.
- **Root app aliases** - added repo-root `npm run dev`, `npm run build`, and
  `npm run tauri -- ...` aliases so the Tauri app can be driven from the repo
...
- **Typed lint cleanup** - removed the existing frontend lint failures that were
  blocking a real enforced lint pass, including unsafe `any` usage, stale
  effect dependencies, and unused props/state.
...
- **Lint cache ignored** - the frontend ESLint cache file is now ignored so
  local lint runs do not dirty the worktree.
...
- **Windows AMD GPU path** - added experimental ROCm support for Cohere
  Transcribe on Windows, including adapter-specific runtimes, setup
  diagnostics, cached GPU wheel installs, and safer fallback behavior. This
  currently works on supported Radeon setups, but can still be slow and remains
  experimental.
- **Issue coverage** - this release closes
  [#32](https://github.com/romeroej2/OpenCassava/issues/32) by shipping the new
  local Cohere Transcribe integration discussed there.
...
- **LLM Unlimited v2 family** - omniASR now uses the `LLM_Unlimited_*_v2` model
  family (`LLM_Unlimited_300M_v2`, `LLM_Unlimited_1B_v2`,
  `LLM_Unlimited_7B_v2`). Settings UI updated to list the new names.
...
- **Venv moved to Linux-native filesystem** - virtual environment now lives under
  ...
  Windows NTFS mount (`/mnt/c/...`). PyTorch `.so` files cannot be loaded by
  the Linux dynamic linker from NTFS; this was the root cause of the
...
- **Reinstall loop eliminated** - `install_runtime` returns early when the
  install stamp is valid, preventing pip from reinstalling everything on every
  launch.
...
- **No temp WAV file** - audio is now passed as a pre-decoded dict directly to
  the pipeline, eliminating the write/read cycle.
- **Model pre-loaded on record** - `ensure_model()` is called immediately after
  the worker spawns so the 1+ GB checkpoint loads up front with log output,
  rather than silently blocking the first audio chunk.
...
- **Correct fairseq2 card names** - the app was using HuggingFace-style paths
  ...
  Correct names are `omniASR_CTC_300M`, `omniASR_CTC_1B`, `omniASR_LLM_300M`,
  etc.
...
- **UTF-8 panic in log pump** - `pump_stderr` panicked when a Unicode
  progress-bar character fell on a non-char-boundary byte offset. Fixed by
  snapping the truncation index to the next valid char boundary.
- **Log line whitespace** - tqdm progress bars pad lines with trailing spaces
  for `\r` overwriting; these are now trimmed before logging.
